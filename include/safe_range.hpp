...
namespace detail {
    ...
    struct check_subtraction_overflow<true, true> {
        ...
        subtract(const T & t, const U & u){
            ...
            if(t > 0 && u < 0){
                if(tmp < 0)
                    boost::numeric::overflow("safe range subtraction result overflow");
            }
            if(t < 0 && u > 0)
                if(tmp >= 0){
                    boost::numeric::overflow("safe range subtraction result underflow");
            }
            return tmp;
        }
    };
    ...
    struct check_subtraction_overflow<false, false> {
        ...
        static BOOST_TYPEOF_TPL(T() + U())
        subtract(const T & t, const U & u) {
            if(boost::numeric::safe_compare::less_than(t, u))
                boost::numeric::overflow("safe range subtraction unsigned difference less than zero");
            return t - u;
        }
    };
    // T unsigned, U signed
    ...
    struct check_subtraction_overflow<false, true> {
        ...
        subtract(const T & t, const U & u){
            ...
            if(boost::numeric::is_unsigned<result_type>::value){
                ...
                // u >= 0
                if(u > t)
                    boost::numeric::overflow("unsigned result is negative");
           }
           // result is signed
           return t - u;
        }
        ...
        subtract(const T & t, const U & u){
            ...
            if(boost::numeric::is_unsigned<result_type>::value){
                return check_subtraction_overflow<false, false>::subtract(
                    boost::numeric::safe_cast<result_type>(t),
                    boost::numeric::safe_cast<result_type>(u)
                );
            }
            // result is signed
    ...
    check_multiplication_overflow(const T & t, const U & u){
        ...
        typedef boost::numeric::bits<boost::uintmax_t> available_bits;

        if(boost::numeric::multiply_result_bits<T, U>::value
        <= boost::numeric::bits<result_type>::value)
            return t * u;

        if(boost::numeric::multiply_result_bits<T, U>::value <= boost::numeric::available_bits::value){
            typedef typename boost::numeric::multiply_result_type<T, U>::type temp_type;
        ...
        if(rt < 0){
            ...
            // address
            if(rt < 0)
                boost::numeric::overflow("overflow of negative value");
        }
        result_type ru = u;
        if(ru < 0){
            ru = ~ru + 1;
        ...
        const int temp_bits = boost::numeric::bits<accumulator_type>::value / 2;
    ...
    check_division_overflow(const T & t, const U & u){
        if(0 == u)
            boost::numeric::overflow("divide by zero");
        ...
        if(boost::numeric::is_signed<U>::value){
            ...
            // both signed
                ...
                if(t == boost::integer_traits<T>::const_min && u == -1)
                    boost::numeric::overflow("overflow in result");
            }
        }
        // both unsigned
        // t signed, u unsigned
    ...
    check_modulus_overflow(const T & t, const U & u){
        if(0 == u)
            boost::numeric::overflow("modulus divide by zero");
